...
function last_list($dbt, $time_delayed)
{
	...
	$sql = "SELECT MAX(wp.mac) AS mac, MAX(wp.ts) AS ts, member.user_login AS login, MAX(member.user_nicename) AS nicename FROM ".$dbt['presence']." AS wp LEFT JOIN (SELECT wpc.mac, ".$dbt['member'].".user_login, ".$dbt['member'].".user_nicename FROM ".$dbt['computer']." AS wpc LEFT JOIN (".$dbt['member'].") ON (wpc.id_member = ".$dbt['member'].".id)) AS member ON (wp.mac = member.mac) WHERE wp.ts IN (SELECT MAX(wp1.ts) FROM ".$dbt['presence']." AS wp1) AND wp.ts > FROM_UNIXTIME(".$time_delayed.") GROUP BY login";
